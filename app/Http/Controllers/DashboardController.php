...
class DashboardController extends Controller
{
    ...
    {
        $activeApplication = Application::with(['state', 'bid' => function ($query) {
            $query->where('user_id', Auth::user()->id);
        }])->where('status', 3)->get();

        $application = Application::with(['state'])->whereHas('bid', function ($query) {
            $query->where('user_id', Auth::user()->id);
        })->where('status', 3)->get();

        $wonApplications = Application::with(['state'])->whereHas('bid', function ($query) {
            $query->where('user_id', Auth::user()->id)->where('status',1);
        })->where('status', 5)->get();

        $lostApplications = Application::with(['state'])->whereHas('bid', function ($query) {
        ...
        $data = [
            ...
            'lostApplications' => $lostApplications,
        ];
        return view('dashboard.lender-applications', $data);
    }
}
